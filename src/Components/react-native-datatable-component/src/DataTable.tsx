...
class DataTable extends React.Component<PropTypes> {
  ...
  handleColPress = name => {
    ...
    if (recentlySortedCol == name) {
      ...
      const data = sortData(
        newData,
        this.state.isSortedAssending[name],
        name,
        true,
      );
      this.setState(state => ({
  ...
  handleOnRowSelect = (isChecked, id, colName) => {
    const data = this.state.data.map(row => {
      ...
      if ('onRowSelect' in this.props)
        this.props?.onRowSelect({...row, [colName]: isChecked}); // Sending props
  ...
  handleNextPreviousPagePress = type => {
    //next | back
    if (type == 'next') {
      // this.state.activeDisplayDataId
      const activeDisplayId = this.state.activeDisplayDataId;
      const endObj = this.state.endDataArray.find(
        obj => obj.id == activeDisplayId + 1,
      );
      const startObj = this.state.startDataArray.find(
        obj => obj.id == activeDisplayId + 1,
      );

      this.setState({
        displayData: this.state.data.slice(
          startObj?.startData - 1,
          endObj?.endData,
        ),
  ...
  static getDerivedStateFromProps(props, currentState) {
    ...
    props.colSettings?.forEach(setting => {
      if (!colNames.includes(setting.name))
        throw new Error(
          'No Column exists which mentioned in provided colSettings prop Name!',
        );
    ...
    return {
      ...
      isSortedAssending: {
        ...currentState.isSortedAssending,
        ...isSortedAssending,
      },
